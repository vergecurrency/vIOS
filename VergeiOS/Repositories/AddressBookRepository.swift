...
class AddressBookRepository {
    ...
    func name(byAddress address: String) -> String? {
        guard let entity =
            ((try? CoreStore.fetchOne(From<AddressType>().where(\.address == address)))
                as AddressType??) else {
            return nil
        }
    ...
    func get(byName name: String) -> Contact? {
        guard let entity =
            ((try? CoreStore.fetchOne(From<AddressType>().where(\.name == name)))
                as AddressType??) else {
            return nil
        }
    ...
    func put(address: Contact) {
        ...
        if let existingEntity =
            ((try? CoreStore.fetchOne(From<AddressType>().where(\.address == address.address)))
                as AddressType??) {
    ...
    func remove(address: Contact) {
        guard let entity =
            ((try? CoreStore.fetchOne(From<AddressType>().where(\.name == address.name)))
                as AddressType??) else {
            return
        }
