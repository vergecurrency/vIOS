...
public final class HDKeychain {
    let privateKey: HDPrivateKey1
    public let curve: DerivationCurve

    public init(privateKey: HDPrivateKey1, curve: DerivationCurve = .secp256k1) {
        self.privateKey = privateKey
        self.curve = curve
    }

    public convenience init(seed: Data, xPrivKey: UInt32, curve: DerivationCurve) {
        self.init(privateKey: HDPrivateKey1(seed: seed, xPrivKey: xPrivKey, salt: curve.bip32SeedSalt), curve: curve)
    }
    ...
    public func derivedKey(path: String) throws -> HDPrivateKey1 {
        var key = privateKey
